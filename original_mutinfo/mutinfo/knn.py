...
class kNN_based(MutualInformationEstimator):
    def __init__(
        self, k_neighbors: int = 1, tree_type: str = "kd_tree", tree_kwargs: dict = {}
    ) -> None:
        """
        ...
        if not tree_type in _metric_tree_types:
            raise ValueError(
                f"The `tree_type` must be in {set(_metric_tree_types.keys())}"
            )
...
class KSG(kNN_based):
    ...
    def __call__(
        self, x: numpy.ndarray, y: numpy.ndarray, std: bool = False
        ...
        x_y_distances = (
            x_y_distances[:, k_neighbors] - 1.0e-15
        )  # Subtract a small epsilon for tolerance.
